...
import type { QuestionStatus, DigitalTestSession, QuestionPayload, StoryPointSection, AdaptiveConfig, AdaptiveState } from '@levelup/shared-types';
import {
  Button,
  AlertDialog,
  AlertDialogAction,
...
export default function TimedTestPage() {
  ...
  const { data: items } = useStoryPointItems(currentTenantId, spaceId ?? null, storyPointId ?? null);
    ...
    if (activeSession) {
      ...
      for (const qId of activeSession.questionOrder) {
        if (activeSession.submissions[qId]) {
          statuses[qId] = activeSession.markedForReview[qId]
            ? 'answered_and_marked'
            : 'answered';
  ...
  const questionOrder = activeSession?.questionOrder ?? items?.filter((i) => i.type === 'question').map((i) => i.id) ?? [];
  ...
  const handleStartTest = async () => {
    ...
    try {
      ...
      const errorCode = (err as { code?: string })?.code ?? (err as { message?: string })?.message ?? '';
  ...
  const handleSaveAnswer = useCallback((itemId: string, answer: unknown) => {
    setAnswers((prev) => ({ ...prev, [itemId]: answer }));
    setQuestionStatuses((prev) => {
      const wasMarked = prev[itemId] === 'marked_for_review' || prev[itemId] === 'answered_and_marked';
      return {
        ...prev,
      ...
      if (adaptiveConfig?.enabled && localAdaptiveState && currentQuestionId) {
        ...
        const remaining: QuestionMeta[] = questionOrder
          ...
          .map((qId) => {
            ...
            return {
              id: qId,
              difficulty: (payload?.difficulty ?? item?.difficulty ?? 'medium') as 'easy' | 'medium' | 'hard',
            };
          });
  ...
  const handleMarkForReview = useCallback(() => {
    ...
    setQuestionStatuses((prev) => {
      ...
      if (current === 'answered' || current === 'answered_and_marked') {
        return { ...prev, [currentQuestionId]: current === 'answered_and_marked' ? 'answered' : 'answered_and_marked' };
      }
      return {
        ...prev,
  ...
  const handleClearResponse = () => {
    ...
    setQuestionStatuses((prev) => ({
      ...prev,
      [currentQuestionId]: prev[currentQuestionId] === 'answered_and_marked' || prev[currentQuestionId] === 'marked_for_review'
        ? 'marked_for_review'
        : 'not_answered',
    }));
  };
  ...
  if (view === 'landing') {
    ...
    const hasPassed = retryConfig?.lockAfterPassing && completedSessions.some(
      (s) => (s.percentage ?? 0) >= passingPct,
    );
    const lastSession = completedSessions.length > 0
      ...
      : null;
    const cooldownEndMs = retryConfig?.cooldownMinutes && lastSession?.endedAt
      ? (lastSession.endedAt as unknown as { seconds: number }).seconds * 1000 + retryConfig.cooldownMinutes * 60 * 1000
      : null;
    const isInCooldown = cooldownEndMs ? now < cooldownEndMs : false;
    const cooldownMinutesLeft = isInCooldown && cooldownEndMs
      ? Math.ceil((cooldownEndMs - now) / 60000)
      : 0;
        ...
        <div className="rounded-lg border bg-card p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Timed Test</p>
            </div>
          </div>

          {/* Schedule banner */}
          {isBeforeSchedule && scheduleStartMs && (
            <div className="rounded bg-blue-50 dark:bg-blue-950/30 p-3 mb-4 text-sm flex items-center gap-2">
          ...
          {isAfterSchedule && (
            <div className="rounded bg-red-50 dark:bg-red-950/30 p-3 mb-4 text-sm flex items-center gap-2">
              <Lock className="h-5 w-5 text-destructive flex-shrink-0" />
              <div>
                <p className="font-medium text-destructive">Closed</p>
          ...
          {/* Cooldown banner */}
          {isInCooldown && (
            <div className="rounded bg-amber-50 dark:bg-amber-950/30 p-3 mb-4 text-sm flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-amber-600/80 dark:text-amber-400/80 text-xs">
                  You can retry in {cooldownMinutesLeft} minute{cooldownMinutesLeft !== 1 ? 's' : ''}.
                </p>
              </div>
            </div>
          )}
          ...
          {hasPassed && (
            <div className="rounded bg-emerald-50 dark:bg-emerald-950/30 p-3 mb-4 text-sm flex items-center gap-2">
          ...
          {storyPoint?.assessmentConfig?.instructions && (
            <div className="rounded bg-primary/5 p-3 mb-4 text-sm">
              <p className="font-medium text-primary mb-1">Instructions:</p>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
            <div className="rounded border p-3">
              ...
              <p className="font-semibold">{storyPoint?.assessmentConfig?.durationMinutes ?? '--'} minutes</p>
            </div>
            <div className="rounded border p-3">
              <p className="text-muted-foreground">Questions</p>
              <p className="font-semibold">{storyPoint?.stats?.totalQuestions ?? items?.filter((i) => i.type === 'question').length ?? '--'}</p>
            </div>
            <div className="rounded border p-3">
            ...
            <div className="rounded border p-3">
              ...
              <p className="font-semibold">{storyPoint?.assessmentConfig?.maxAttempts ?? 'Unlimited'}</p>
            </div>
          </div>
          ...
          {storyPoint?.assessmentConfig?.adaptiveConfig?.enabled && (
            <div className="rounded bg-blue-50 dark:bg-blue-950/30 p-3 mb-4 text-sm">
          ...
          <Button onClick={handleStartTest} disabled={startTest.isPending || !canStart} className="gap-2">
            <PlayCircle className="h-5 w-5" />
            {startTest.isPending ? 'Starting...' : !canStart ? 'Unavailable' : 'Start Test'}
          </Button>

          {startError && (
            <p className="mt-2 text-sm text-destructive" data-testid="start-error">
              {startError}
            </p>
          )}
        ...
        {completedSessions.length > 0 && (
          <div className="space-y-3">
            ...
            {completedSessions.map((session, idx) => {
              ...
              const improved = prevSession && (session.percentage ?? 0) > (prevSession.percentage ?? 0);

              return (
                <button
                  key={session.id}
                  onClick={() => { setSelectedSession(session); setView('results'); }}
                  className="w-full text-left rounded-lg border p-4 hover:shadow-sm transition-shadow"
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {session.endedAt ? new Date(session.endedAt.seconds * 1000).toLocaleDateString() : '--'}
                      </p>
                    </div>
  ...
  if (view === 'test' && activeSession) {
    ...
    const answeredCount = Object.values(questionStatuses).filter((s) => s === 'answered' || s === 'answered_and_marked').length;
    const markedCount = Object.values(questionStatuses).filter((s) => s === 'marked_for_review' || s === 'answered_and_marked').length;
    const unansweredCount = Object.values(questionStatuses).filter((s) => s === 'not_answered' || s === 'not_visited').length;
    ...
    return (
      <div className="flex gap-4 h-[calc(100vh-4rem)]">
        ...
        {/* Center: Question */}
        <main ref={mainRef} tabIndex={-1} className="flex-1 min-w-0 overflow-y-auto outline-none">
          ...
          <div className="flex items-center justify-between mb-4 sticky top-0 bg-background z-10 py-2 border-b">
            ...
            <CountdownTimer deadline={deadline} onTimeUp={handleTimeUp} serverOffset={serverTimeOffset} />
          </div>
        ...
        <AlertDialog open={showConfirm} onOpenChange={setShowConfirm}>
          <AlertDialogContent>
            ...
            <div className="text-sm space-y-1">
              ...
              {unansweredCount === 0 && (
                <p className="text-emerald-600">All questions answered</p>
              )}
            </div>
            <AlertDialogFooter>
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink asChild><Link to={`/spaces/${spaceId}`}>{space?.title}</Link></BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
        ...
        {autoSubmitNotice && session.autoSubmitted && (
          <div className="rounded-md bg-amber-500/10 border border-amber-200 dark:border-amber-800 p-3 text-sm text-amber-700 dark:text-amber-400" role="alert">
            Time expired — your test was auto-submitted.
          </div>
        )}

        <div className="rounded-lg border bg-card p-6 text-center">
          <Award className={`h-12 w-12 mx-auto mb-3 ${passed ? 'text-emerald-500' : 'text-destructive'}`} />
          ...
          {improvement != null && improvement !== 0 && (
            <p className={`text-sm mt-1 ${improvement > 0 ? 'text-emerald-600' : 'text-destructive'}`}>
              {improvement > 0 ? '+' : ''}{Math.round(improvement)}% vs previous attempt
            </p>
          )}
        ...
        {session.analytics?.difficultyBreakdown && Object.keys(session.analytics.difficultyBreakdown).length > 0 && (
          <div className="space-y-3">
        ...
        {session.analytics?.bloomsBreakdown && Object.keys(session.analytics.bloomsBreakdown).length > 0 && (
          <div className="space-y-3">
            ...
            {Object.entries(session.analytics.bloomsBreakdown).map(([level, data]) => (
              <div key={level} className="flex items-center gap-3">
                <span className="text-sm w-32 truncate capitalize">{level}</span>
                <div className="flex-1">
                  <ProgressBar
                    value={data.correct}
                    max={data.total}
            ...
            {Object.entries(session.analytics.sectionBreakdown).map(([sectionId, data]) => {
              ...
              return (
                <div key={sectionId} className="flex items-center gap-3">
                  <span className="text-sm w-32 truncate">{section?.title ?? sectionId}</span>
                  <div className="flex-1">
                    <ProgressBar
                      value={data.correct}
                      max={data.total}
            ...
            return (
              <div key={qId} className="rounded-lg border p-4">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium">Q{index + 1}: {payload?.title ?? payload?.content?.slice(0, 60) ?? 'Question'}</span>
                    {qSection && (
                  ...
                  {submission?.correct != null && (
                    <span className={`text-xs font-medium ${submission.correct ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive'}`}>
                ...
                {submission && (
                  <div className="text-xs text-muted-foreground">
                    ...
                    {submission.timeSpentSeconds > 0 && ` | Time: ${Math.round(submission.timeSpentSeconds)}s`}
                  </div>
                )}
                {!submission && (
        ...
        {/* Topic Analysis */}
        {session.analytics?.topicBreakdown && Object.keys(session.analytics.topicBreakdown).length > 0 && (
          <div className="space-y-3">
          ...
          return (
            <div className="rounded-lg border bg-amber-50/50 dark:bg-amber-950/20 p-4 space-y-2">
              ...
              {weakTopics.map(([topic, data]) => (
                <p key={topic} className="text-sm text-muted-foreground">
                  Focus on <span className="font-medium text-foreground">{topic}</span>{' '}
                  ({data.correct}/{data.total} correct)
                </p>
              ))}
              <Button variant="link" size="sm" asChild className="px-0 h-auto">
                <Link to={`/spaces/${spaceId}/test/${storyPointId}/analytics`}>
                  <BarChart3 className="h-3 w-3 mr-1" /> View full analytics
                </Link>
              </Button>
            </div>
          );
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-end gap-1 h-16">
              {session.difficultyProgression.map((entry, idx) => {
                const height = entry.difficulty === 'hard' ? 100 : entry.difficulty === 'medium' ? 60 : 30;
                return (
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            onClick={() => { setView('landing'); setSelectedSession(null); }}
          >
            Back to Test Info
          </Button>
...
function AnimatedScoreGrid({ session }: { session: DigitalTestSession }) {
  const animatedScore = useCountUp(
    session.percentage != null ? Math.round(session.percentage) : 0,
  );
