  ...
  return useQuery<SpaceAnalytics | null>({
    ...
    queryFn: async () => {
      ...
      const colRef = collection(
        db,
        `tenants/${tenantId}/spaces/${spaceId}/progress`
      );
      ...
      const avgEngagementMinutes =
        progresses.reduce(
          (sum, p) => sum + (p.totalTimeMinutes ?? 0),
          0
        ) / totalStudents;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <SelectContent>
            {spaces.length === 0 && (
              <SelectItem value="__none__" disabled>No published spaces</SelectItem>
            )}
            {spaces.map((s: Space) => (
            ...
            <div className="rounded-lg border bg-card p-5 space-y-4">
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="text-sm space-y-2">
                  <p>
                    Space:{" "}
                    <span className="font-medium">{activeSpace?.title}</span>
                  </p>
                  <p>
                    Type:{" "}
                  ...
                  {activeSpace?.subject && (
                    <p>
                      Subject:{" "}
                      <span className="font-medium">{activeSpace.subject}</span>
                    </p>
                  )}
                  <p>
                    Story Points:{" "}
                    <span className="font-medium">
                      {activeSpace?.stats?.totalStoryPoints ?? 0}
                    </span>
                  </p>
                </div>
              </div>
