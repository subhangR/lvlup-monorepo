...
1. **Consolidate navigation** — merge Space Progress and Exam Results into Child Progress as tabs to reduce cognitive load (N1)
2. **Add offline-aware error states** — detect offline and show appropriate messaging instead of empty/broken states (X1)
3. **Add actionable links in alerts** — connect alert items to relevant detail pages (PA1)
4. **Add sorting/filtering to data-heavy pages** — Exam Results, Space Progress need basic sort/filter controls (E1, S1)
5. **Fix mobile navigation gaps** — add "More" menu to mobile bottom nav to surface hidden pages (N2)
