...
## Overview
Built the rule-based Insight Engine for personalized learning recommendations, Exam-Space linkage for the study→assessment flow, and integrated both into student and teacher UIs.
...
- `exam_preparation` — Upcoming exam has linked Space that student hasn't completed
...
- `cross_system_correlation` — Students who completed linked Space scored significantly higher
...
**Constraints:** Max 5 active insights per student. Priority sort: high → medium → low.
...
2. **Rule engine is pure functions**: `insight-rules.ts` exports pure functions that take data and return insight seeds. No Firestore calls inside rules — all data is pre-fetched by the scheduler. This makes rules testable and composable.
