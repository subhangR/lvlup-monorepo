...
- **`apps/student-web/src/App.tsx`** — Added `/settings` and `/profile` lazy-loaded routes
- **`apps/student-web/src/layouts/AppLayout.tsx`** — Added "Account" nav group with Profile (UserCircle icon) and Settings (Settings icon)

### A4. Student notification hooks (NEW)
- `apps/student-web/src/hooks/useNotificationPreferences.ts` — Reads from `tenants/{tenantId}/notificationPreferences/{userId}`
- `apps/student-web/src/hooks/useSaveNotificationPreferences.ts` — Writes prefs with merge, invalidates query cache
...
- **`apps/parent-web/src/pages/ChildProgressPage.tsx`** — Added `DownloadPDFButton` to page header
...
- **`apps/parent-web/src/layouts/AppLayout.tsx`** — Added "Compare Children" nav item (BarChart3 icon) in My Children group
...
- **`apps/teacher-web/src/pages/StudentReportPage.tsx`** — Added `DownloadPDFButton` to page header
...
- **`apps/teacher-web/src/layouts/AppLayout.tsx`** — Added "Batch Grading" nav item (CheckSquare icon) in Content group
...
- CelebrationBurst particle colors are intentionally vivid (overlay) — no dark mode change needed
