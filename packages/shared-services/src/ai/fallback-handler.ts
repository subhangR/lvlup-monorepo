...
export function classifyError(err: unknown): ClassifiedError {
  ...
  if (messageLower.includes('429') || messageLower.includes('resource exhausted') || messageLower.includes('quota')) {
    return {
      type: 'quota',
  ...
  if (messageLower.includes('401') || messageLower.includes('403') || messageLower.includes('invalid api key') || messageLower.includes('permission denied')) {
    return {
      type: 'auth',
  ...
  // Model errors
  if (messageLower.includes('model not found') || messageLower.includes('not supported') || messageLower.includes('deprecated')) {
    return {
      type: 'model',
  ...
  // Timeout
  if (messageLower.includes('timeout') || messageLower.includes('deadline exceeded') || messageLower.includes('timed out')) {
    return {
      type: 'timeout',
  ...
  if (messageLower.includes('500') || messageLower.includes('502') || messageLower.includes('503') || messageLower.includes('504') || messageLower.includes('overloaded') || messageLower.includes('internal')) {
    return {
      type: 'transient',
...
export function recordFailure(tenantId: string): void {
  ...
  if (state.failures >= CIRCUIT_BREAKER_THRESHOLD) {
    state.isOpen = true;
    console.warn(`[CircuitBreaker] Circuit opened for tenant ${tenantId} after ${state.failures} failures`);
  }
