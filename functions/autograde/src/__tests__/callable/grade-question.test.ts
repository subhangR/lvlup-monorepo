...
describe("gradeQuestion callable", () => {
  ...
  it("rejects when tenantId is missing", async () => {
    await expect((gradeQuestion as any)(makeRequest({ mode: "manual" }))).rejects.toThrow(
      "Missing required fields"
    );
  });

  it("rejects when mode is missing", async () => {
    await expect((gradeQuestion as any)(makeRequest({ tenantId: "tenant-1" }))).rejects.toThrow(
      "Missing required fields"
    );
  });
  ...
  it("creates new question submission for manual grade when none exists", async () => {
    ...
    const result = await (gradeQuestion as any)(
      makeRequest({
        tenantId: "tenant-1",
        mode: "manual",
        submissionId: "s1",
  ...
  it("overrides existing graded question submission", async () => {
    ...
    mockCollectionGet.mockResolvedValue({
      docs: [
        { id: "q1", data: () => ({ gradingStatus: "overridden", manualOverride: { score: 9 } }) },
      ],
    });
