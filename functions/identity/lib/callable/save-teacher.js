 ...
 */
exports.saveTeacher = (0, https_1.onCall)({ region: 'asia-south1', cors: true }, async (request) => {
    const callerUid = request.auth?.uid;
    if (!callerUid)
        throw new https_1.HttpsError('unauthenticated', 'Must be logged in');
    const { id, tenantId, data } = (0, utils_1.parseRequest)(request.data, shared_types_2.SaveTeacherRequestSchema);
    ...
    if (!id) {
        ...
        if (data.permissions !== undefined) {
            ...
            if (membershipDoc.exists && membershipDoc.data()?.role === 'teacher') {
                ...
                const classIdsChanged = JSON.stringify(currentPerms.managedClassIds) !==
