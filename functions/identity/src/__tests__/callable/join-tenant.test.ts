...
const mockDocRef = { get: mockGet, set: mockSet, update: mockUpdate, create: mockCreate, id: 'auto-id' };
...
vi.mock('firebase-admin', () => {
  ...
  return {
    default: {
      firestore: fsFn,
      auth: () => ({ setCustomUserClaims: vi.fn(), getUser: vi.fn().mockResolvedValue({ uid: 'user-1' }) }),
      initializeApp: vi.fn(),
    },
    firestore: fsFn,
    auth: () => ({ setCustomUserClaims: vi.fn(), getUser: vi.fn().mockResolvedValue({ uid: 'user-1' }) }),
...
describe('joinTenant', () => {
  ...
  it('should throw unauthenticated when no auth', async () => {
    await expect(handler(makeRequest({ code: 'GRN001' }, null))).rejects.toThrow();
  });
  ...
  it('should throw when tenant code not found', async () => {
    ...
    await expect(handler(makeRequest({ code: 'INVALID' }))).rejects.toThrow();
  });
  ...
  it('should create membership when valid tenant code provided', async () => {
    ...
    const result = await handler(makeRequest({ code: 'GRN001' }));
  ...
  it('should reactivate deactivated membership', async () => {
    ...
    // Existing deactivated membership found
    mockGet.mockResolvedValueOnce({
      empty: false,
      docs: [
        {
          id: 'mem-1',
          ref: mockDocRef,
          data: () => ({ uid: 'user-1', tenantId: 'tenant-1', status: 'deactivated', role: 'student' }),
        },
      ],
    });
