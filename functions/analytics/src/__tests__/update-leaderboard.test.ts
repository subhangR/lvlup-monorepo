...
function makeEvent(afterData: Record<string, any> | null, params: Record<string, string> = {}) {
  return {
    data: {
      before: { data: () => null },
...
describe("updateLeaderboard — tier counts", () => {
  ...
  it("should compute tier counts from subject breakdown", async () => {
    ...
    const tierCounts = rtdbUpdates[0]["tenantLeaderboard/tenant-1/student-1/countsByTier"];
    expect(tierCounts).toBeDefined();
    expect(tierCounts.diamond).toBe(1);
  ...
  it("should return all zeros when subjects are below threshold", async () => {
    ...
    const tierCounts = rtdbUpdates[0]["tenantLeaderboard/tenant-1/student-1/countsByTier"];
    expect(tierCounts.diamond).toBe(0);
    expect(tierCounts.platinum).toBe(0);
    expect(tierCounts.gold).toBe(0);
    expect(tierCounts.silver).toBe(0);
  });
