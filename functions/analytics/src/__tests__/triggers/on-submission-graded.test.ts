...
function makeEvent(beforeData: any, afterData: any) {
  return {
    ...
    data: {
      ...
      after: { data: () => afterData, ref: { path: `tenants/${TENANT}/submissions/${SUBMISSION_ID}` } },
    },
  };
}
...
describe('onSubmissionGraded', () => {
  ...
  it('should process when pipelineStatus transitions to graded', async () => {
    ...
    mockGet.mockResolvedValueOnce({
      docs: [
        { id: 'sub-1', data: () => ({ examId: 'exam-1', totalScore: 80, maxScore: 100, pipelineStatus: 'graded' }) },
      ],
    });
    ...
    mockGet.mockResolvedValueOnce({
      docs: [
        { id: 'exam-1', data: () => ({ subject: 'Math', title: 'Test 1', maxScore: 100 }) },
      ],
    });

    mockRunTransaction.mockImplementation(async (fn: any) => {
      const txn = { get: vi.fn().mockResolvedValue({ exists: true, data: () => ({}) }), set: vi.fn(), update: vi.fn() };
      await fn(txn);
    });
  ...
  it('should handle grading_complete status transition', async () => {
    const event = makeEvent(
      { pipelineStatus: 'graded', studentUid: 'stu-1', examId: 'exam-1' },
