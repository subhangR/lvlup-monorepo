...
vi.mock('../utils/pdf-helpers', () => ({
  ...
  COLORS: {
    primary: '#1e40af', secondary: '#6b7280', accent: '#059669',
    danger: '#dc2626', warning: '#d97706', headerBg: '#1e3a5f',
    lightGray: '#f3f4f6', border: '#d1d5db', text: '#111827', muted: '#6b7280',
  },
}));
...
describe('generateReport — input validation', () => {
  ...
  it('should reject invalid type value', async () => {
    ...
    await expect(handler(req)).rejects.toThrow('type must be "exam-result", "progress", or "class"');
  });
});
...
describe('generateReport — exam-result (individual)', () => {
  ...
  it('should return not-found for missing exam', async () => {
    ...
    const req = makeRequest({ tenantId: TENANT, type: 'exam-result', examId: 'exam-1', studentId: 'student-1' });
...
describe('generateReport — exam-result (class summary)', () => {
  ...
  it('should generate class exam summary report when no studentId', async () => {
    ...
    expect(mockBucket.file).toHaveBeenCalledWith(
      expect.stringContaining('class-summary.pdf'),
    );
  });
...
describe('generateReport — progress', () => {
  ...
  it('should generate progress PDF and upload to storage', async () => {
    ...
    mockFirestore.seedCollection('userMemberships', [
      { id: 'um1', data: { tenantId: TENANT, uid: 'student-1', role: 'student', firstName: 'Alice', lastName: 'Smith' } },
    ]);
...
describe('generateReport — class', () => {
  ...
  it('should generate class report card PDF', async () => {
    ...
    mockFirestore.seedCollection('userMemberships', [
      { id: 'um1', data: { tenantId: TENANT, uid: 's1', role: 'student', classIds: ['class-1'], firstName: 'Alice', lastName: 'Smith' } },
    ]);
    ...
    mockFirestore.seedDoc(`tenants/${TENANT}/studentProgressSummaries/s1`, {
      ...
      isAtRisk: false,
      atRiskReasons: [],
      autograde: { totalExams: 3, averagePercentage: 72, totalMarksObtained: 216, totalMarksAvailable: 300 },
      levelup: { totalSpaces: 2, completedSpaces: 1, averageCompletion: 65, totalPointsEarned: 100 },
    });
...
describe('generateReport — storage upload', () => {
  ...
  it('should upload with correct content type', async () => {
    ...
    mockFirestore.seedDoc(`tenants/${TENANT}/studentProgressSummaries/student-1`, {
      ...
      weaknessAreas: [],
      autograde: {
        totalExams: 0, completedExams: 0, averagePercentage: 0,
        totalMarksObtained: 0, totalMarksAvailable: 0,
        subjectBreakdown: {}, recentExams: [],
      },
      levelup: {
        totalSpaces: 0, completedSpaces: 0, averageCompletion: 0,
        totalPointsEarned: 0, totalPointsAvailable: 0,
        averageAccuracy: 0, streakDays: 0, subjectBreakdown: {},
      },
    });
    mockFirestore.seedCollection('userMemberships', []);
